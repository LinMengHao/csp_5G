package com.xzkj.taskProxy.sms.handler.models.query;
...
public class HandlerModelQueryToHLGYD implements Runnable {
	...
	public HandlerModelQueryToHLGYD(String companyId, String appId, String channelId, int curCount) {
...
//					httppost.setHeader("Content-Type", "multipart/form-data;charset=utf-8");
				...
				if("0".equals(resCode)){
					...
					logger.info("updateSql: {}",updateSql);
					//删除模版映射关系
					String deleteSql = String.format("update e_model_related set status=%s where model_id='%s' and channel_model_id='%s' and channel_id=%s;",2,pModelId,channelModelId,channelId);
