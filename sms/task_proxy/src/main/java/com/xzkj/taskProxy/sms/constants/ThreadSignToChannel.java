...
import com.xzkj.taskProxy.sms.handler.sign.HandlerSignToHLGYD;
import com.xzkj.taskProxy.sms.handler.sign.HandlerSignToLiDinYD;
import com.xzkj.taskProxy.sms.handler.sign.HandlerSignToLianTong;
import com.xzkj.taskProxy.sms.handler.sign.HandlerSignToTaSiYD;
...
public class ThreadSignToChannel extends Thread{
	public static Logger logger = LoggerFactory.getLogger("ThreadSignToChannel");
	...
	public void run() { 
		...
		while(true){
			...
			for (Map.Entry<String,String> en : map.entrySet()) {
				...
				switch (channelId) {
					case "1"://beijing cmcc
						BusinessWorker.getExecutor().execute(new HandlerSignToLiDinYD(companyId, app_id, channelId,curCount));
						break;
					case "2"://beijing cmcc2
						BusinessWorker.getExecutor().execute(new HandlerSignToTaSiYD(companyId, app_id, channelId,curCount));
						break;
					case "3"://陕西联通
						BusinessWorker.getExecutor().execute(new HandlerSignToLianTong(companyId, app_id, channelId,curCount));
						break;
...
//						BusinessWorker.getExecutor().execute(new HandlerModelToShanghaiDianXin(companyId, app_id, channelId,curCount));
					...
					case "6"://beijing cmcc3
						BusinessWorker.getExecutor().execute(new HandlerSignToHLGYD(companyId, app_id, channelId,curCount));
						break;
					case "7"://修治上海电信
...
//						BusinessWorker.getExecutor().execute(new HandlerModelToWuLangTwo(companyId, app_id, channelId,curCount));
			...
			try{
				...
				logger.error("ThreadSignToChannel error:string_exist:", e);
			}
			try {
