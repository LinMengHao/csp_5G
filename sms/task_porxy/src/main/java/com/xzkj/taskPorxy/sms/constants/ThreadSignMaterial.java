...
import com.xzkj.taskPorxy.sms.handler.HandlerSignMaterial;
...
public class ThreadSignMaterial extends Thread{
	public static Logger logger = LoggerFactory.getLogger("ThreadSignMaterial");

	public void run() { 
		logger.info("开始执行视频短信签名处理任务。。。");
		...
		while(true){
			...
			for (Map.Entry<String,String> en : map.entrySet()) {
				...
				BusinessWorker.getExecutor().execute(new HandlerSignMaterial(companyId, app_id, curCount));
			...
			try{
				...
				logger.error("ThreadSignMaterial error:string_exist:", e);
			}
			try {
