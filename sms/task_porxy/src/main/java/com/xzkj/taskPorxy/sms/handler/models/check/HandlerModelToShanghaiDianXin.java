...
public class HandlerModelToShanghaiDianXin implements Runnable {
	public static Logger logger = LoggerFactory.getLogger("HandlerModelToShanghaiDianXin");
	...
	public HandlerModelToShanghaiDianXin(String companyId, String appId, String channelId,int curCount) {
		...
		try {
			...
			while((jsonStr = RedisUtils.fifo_pop(fifo_key))!=null){
				...
				try{
					...
					String CLIENT_SIID="22100670010006";
					String CLIENT_KEY="ViEW4aoQNU";
					...
					//扩展号码
					reqElementsJsonObject.put("ExtNum", extNum);
					//内容主题
					...
					logger.info("上海电信模版提交请求地址: {}" ,httppost.getRequestLine());
					CloseableHttpResponse response = httpclient.execute(httppost);
					logger.info("上海电信模版提交响应 : {}" ,response);
...
//					if(modelId.length()<=11){
						...
						logger.info("updateSql1: {}",updateSql);
//					}else {
//						//修改模版审核状态
