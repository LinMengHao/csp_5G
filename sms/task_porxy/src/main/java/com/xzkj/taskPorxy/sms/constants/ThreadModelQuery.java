...
import com.xzkj.taskPorxy.sms.handler.models.query.HandlerModelQueryToHLGYD;
import com.xzkj.taskPorxy.sms.handler.models.query.HandlerModelQueryToLiDinYD;
import com.xzkj.taskPorxy.sms.handler.models.query.HandlerModelQueryToTaSiYD;
...
public class ThreadModelQuery extends Thread{
	public static Logger logger = LoggerFactory.getLogger("ThreadModelQuery");
	...
	public void run() { 
		...
		while(true){
			...
			for (Map.Entry<String,String> en : map.entrySet()) {
				...
				switch (channelId) {
					case "1"://beijing cmcc
						BusinessWorker.getExecutor().execute(new HandlerModelQueryToLiDinYD(companyId, app_id, channelId,curCount));
						break;
					case "2"://beijing cmcc2
						BusinessWorker.getExecutor().execute(new HandlerModelQueryToTaSiYD(companyId, app_id, channelId,curCount));
						break;
					case "3"://陕西联通
...
//						BusinessWorker.getExecutor().execute(new HandlerModelToShanghaiDianXin(companyId, app_id, channelId,curCount));
					...
					case "6"://beijing cmcc3
						BusinessWorker.getExecutor().execute(new HandlerModelQueryToHLGYD(companyId, app_id, channelId,curCount));
						break;
					case "7"://修治上海电信
