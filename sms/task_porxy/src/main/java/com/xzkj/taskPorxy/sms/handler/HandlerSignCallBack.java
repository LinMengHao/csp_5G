...
public class HandlerSignCallBack implements Runnable {
public static Logger logger = LoggerFactory.getLogger("HandlerSignCallBack");

	private String companyId;
	private int total;

	public HandlerSignCallBack(String companyId, int total) throws IOException{
	...
	public void run(){
		...
		*/
		...
		String fifo_back_key = RedisUtils.FIFO_SIGN_MT_CLIENT+this.companyId;
		...
		while((jsonString = RedisUtils.fifo_pop(fifo_back_key))!=null){
			...
			if(StringUtils.isNotBlank(jsonString)){
				try{
					...
					logger.info("HandlerSignCallBack jsonString【{}】 操作异常:{}",jsonString, e);
					continue;
				}
			}
			if(size>=this.total){
		...
		String hash_back_key = RedisUtils.HASH_SIGN_MT_COUNT;
...
//				String password = json.getString("password");
				String reportSignContent = json.getString("reportSignContent");
				String signId = json.getString("signId");
...
//				}
				...
				sb.append(appName).append("&signId=").append(signId)
				...
				paramJson.put("signId", signId);
				paramJson.put("reportSignContent", reportSignContent);
				...
				jsonArr = map.containsKey(backSign)?map.get(backSign):new JSONArray();
				jsonArr.add(paramJson);
				jsonArray.add(json);
				map.put(backSign,jsonArr);
			}
			...
			for(Map.Entry<String,JSONArray> en:map.entrySet()){
				...
				if(!httpResults.equals("000")){
					...
					RedisUtils.zset_set(RedisUtils.ZSET_KEY_BACK_MORE+"signRobot", Double.parseDouble(format),jsonMore.toString());
					...
					for (int i = 0 ;i < jsonArrs.size(); i++) {
						...
						//数据库状态同步更新
						//修改模版审核状态
						String updateSql = String.format("update e_model_sign set status='%s',info='%s' where id='%s';",status,info,id);
				...
				logger.info("HandlerSignCallBack线程内：外呼回调通知地址({})参数({})，cbType={},回调通知客户返回值:{}",callUrl,logparamter,cbType,httpResults);

			}

		}catch (Exception e1) {
			logger.info("HandlerSignCallBack线程内：外呼回调通知异常：{}",e1.getMessage());
			e1.printStackTrace();
		}
	}
