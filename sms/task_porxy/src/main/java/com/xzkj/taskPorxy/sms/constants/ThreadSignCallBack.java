...
import com.xzkj.taskPorxy.sms.handler.HandlerSignCallBack;
...
public class ThreadSignCallBack extends Thread{
	public static Logger logger = LoggerFactory.getLogger("ThreadSignCallBack");

	public void run() { 
		logger.info("开始执行签名回调通知任务。。。");
		int i=0;
		RedisUtils.string_set(RedisUtils.STR_KEY_AI_TASK,"run");
		while(true){
			//long bt = System.currentTimeMillis();
			String hash_sql_key = RedisUtils.HASH_SIGN_MT_COUNT;
			...
			for (Map.Entry<String,String> en : map.entrySet()) {
		        ...
		        try {
		        	...
		        	BusinessWorker.getExecutor().execute(new HandlerSignCallBack(companyId,total));
		        } catch (Exception e) {
		        	logger.error("ThreadSignCallBack error", e);
		        } 
			}
			/*i++;
			if(i%1000==0){
			...
			try{
				...
				logger.error("ThreadSignCallBack error:string_exist:", e);
			}
			try {
				Thread.sleep(1000);
